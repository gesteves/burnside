...
* I'm trying to simplify the process of creating new posts and deploying to Heroku. I'm currently trying to figure out how to run the rake tasks (`new_post`, `generate`, and `deploy`) using Automator, to add them to OS X's Services menu. I also replaced some of the `puts` calls in the Rakefile with calls to `growlnotify`, to get nice Growl notifications on successful deploys and whatnot.
