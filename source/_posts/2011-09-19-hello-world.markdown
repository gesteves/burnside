...
* I'm trying to simplify the process of creating new posts and deploying to Heroku, and integrate it better to the OS. For example, I added the deploy task to OS X's Services menu, using Automator to create a new service that simply calls the rake task from a shell script. I also replaced some of the `puts` calls in the Rakefile with calls to `growlnotify`, to get nice Growl notifications on successful deploys and whatnot.
